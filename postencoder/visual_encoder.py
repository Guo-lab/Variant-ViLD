...
def process_image(CONFIG):
    ...
    ViLDmodel = tf.saved_model.loader.load(session, ['serve'], CONFIG.saved_model_dir)
    ####################################################
    ##########################################
    roi_boxes, roi_scores, \
    detection_boxes, \
    scores_unused, box_outputs, \
    detection_masks, \
    visual_features, \
    image_info = session.run(
        ['RoiBoxes:0', 
         'RoiScores:0', 
         '2ndStageBoxes:0', 
    ...
    #//constant_graph = graph_util.convert_variables_to_constants(session, session.graph_def,)
    #//output_graph = 'model.pb' 
    #//with tf.gfile.GFile(output_graph, 'wb') as f:
    #//    f.write(constant_graph.SerializeToString())
    #//from tensorflow.python.tools import freeze_graph
    #//tf.train.write_graph(session.graph_def, 'output_model/pb_model', 'model.pb')
    tf.train.write_graph(session.graph_def, "./checkGraph", 'graph.pbtxt', as_text=False)
    pdb.set_trace()
    ##########################################
    ###################################################
    
    roi_boxes = np.squeeze(roi_boxes, axis=0)  # squeeze
    roi_scores = np.squeeze(roi_scores, axis=0) # no need to clip the boxes, already done
